...
const getResponsiveMargins = (containerWidth: number) => {
  if (containerWidth <= MOBILE_BREAKPOINT) {
    ...
    return {
      topMargin: 0.1, // Smaller top margin on tablet
      sideMargin: 0.13,
    };
  } else {
    return {
...
export function ScheduleVisualization({
  ...
  const drawSchedule = useCallback(
    (scheduleIndex: number) => {
      ...
      const isCurrentlyFavorited = favoritedCombinations.has(
        schedule.combination_id.toString(),
      );
      ...
      scheduleCourses.forEach((course) => {
        ...
        course.sessions.forEach((session) => {
          ...
          const startY =
            topMarginOffset +
            (hourHeight * (sessionStartMinutes - startTime)) / 60;
          const height =
            ((sessionEndMinutes - sessionStartMinutes) * hourHeight) / 60;
        ...
        blocks.forEach((block) => {
          ...
          const drawFittingText = (
            ...
            if (textWidth > maxWidth) {
              ...
              displayText =
                start > 3
                  ? text.substring(0, start - 3) + "..."
                  : text.substring(0, Math.max(1, start));
            }
          ...
          ctx.fillStyle = "#ffffff";
          const nameHeight = drawFittingText(
            course.course_name,
            textX,
            currentY,
            maxWidth,
            fontSizes.courseFont + 1,
            "bold",
          );
          currentY += nameHeight;
          ...
          if (currentY + fontSizes.courseFont + 2 < yPos + maxTextHeight) {
            ...
            const codeHeight = drawFittingText(
              course.course_code,
              textX,
              currentY,
              maxWidth,
          ...
          if (currentY + fontSizes.courseFont + 2 < yPos + maxTextHeight) {
            ...
            const sectionHeight = drawFittingText(
              sectionText,
              textX,
              currentY,
              maxWidth,
              fontSizes.courseFont - 2,
              "normal",
            );
          ...
          if (
            blockHeight > 50 &&
            currentY + fontSizes.timeFont + 2 < yPos + maxTextHeight
          ) {
            ctx.fillStyle = "#ffffff";
          ...
          if (
            session.location &&
            blockHeight > 65 &&
            currentY + fontSizes.locationFont + 2 < yPos + maxTextHeight
          ) {
  ...
  useEffect(() => {
    ...
    const handleCanvasClick = (event: MouseEvent) => {
      ...
      const clickedBlock = clickableBlocks.find(
        (block) =>
          scaledX >= block.x &&
          scaledX <= block.x + block.width &&
          scaledY >= block.y &&
          scaledY <= block.y + block.height,
      );
    ...
    const handleCanvasMouseMove = (event: MouseEvent) => {
      ...
      const hoveredBlock = clickableBlocks.find(
        (block) =>
          scaledX >= block.x &&
          scaledX <= block.x + block.width &&
          scaledY >= block.y &&
  ...
  const toggleFavorite = () => {
    ...
    const isFavorited = favoritedCombinations.has(
      currentSchedule.combination_id,
    );
  ...
  const generateImageForIndex = async (index: number) => {
    if (index < 0 || index >= combinations.length || scheduleImages[index])
      return;
        ...
        <CardContent>
          <div className="text-center py-8 text-muted-foreground">
            ...
            <p className="text-sm mt-2">
              Intenta seleccionar más secciones o verifica que no haya
  ...
  return (
    <>
      <Card className="bg-card/80 backdrop-blur-sm border-border shadow-lg transition-all duration-300 hover:shadow-xl sm:hover:-translate-y-1">
        <CardHeader className="pb-4">
          ...
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <CardTitle className="flex items-center gap-2 text-foreground text-lg tracking-tight">
                  <Calendar className="w-5 h-5" />
                  {scheduleName || "Horarios Generados"}
                </CardTitle>
            ...
            <div className="flex items-center gap-2 shrink-0">
              {(onAddToFavorites || onRemoveFromFavorites) && (
                <Button
                  ...
                  variant={
                    favoritedCombinations.has(
                      combinations[currentScheduleIndex]?.combination_id,
                    )
                      ? "default"
                      : "outline"
                  }
                  className={
                    favoritedCombinations.has(
                      ...
                      ? "bg-foreground text-background hover:bg-foreground/90 border-0 shadow-sm"
                      : "border-border text-foreground hover:bg-muted"
                  }
                >
                  <Heart
                    className={`w-4 h-4 mr-1.5 ${favoritedCombinations.has(combinations[currentScheduleIndex]?.combination_id) ? "fill-current" : ""}`}
                  />
                  {favoritedCombinations.has(
                    combinations[currentScheduleIndex]?.combination_id,
                  )
        ...
        <CardContent className="pt-0">
          <div className="space-y-4">
            ...
            <div
              ref={containerRef}
              className={`border border-border rounded-lg bg-card shadow-inner ${
            ...
            <div className="flex items-center gap-2 text-xs text-muted-foreground">
              ...
              <input
                ...
                className="px-1.5 py-1 border border-border rounded bg-background text-foreground text-xs w-[5.5rem]"
              />
      ...
      {isImageModalOpen && modalImageUrl && (
        ...
        >
          ...
          >
            ...
            <button
              onClick={downloadSchedule}
              className="absolute bottom-4 right-4 bg-primary hover:bg-primary/90 text-primary-foreground px-4 py-3 rounded-lg shadow-lg transition-colors flex items-center gap-2"
            >
            ...
            <div className="absolute top-4 left-4 bg-black/80 text-white px-4 py-2 rounded-lg">
              ...
              <div className="text-xs text-gray-300">
                {combinations[currentScheduleIndex]?.courses.length || 0} cursos
      ...
      {selectedCourse && (
        ...
        >
          <div
            className="bg-card/95 backdrop-blur-md border border-border/50 rounded-xl p-6 sm:p-8 mx-4 shadow-2xl transform transition-all duration-300 scale-100 max-h-[90vh] overflow-y-auto relative"
            onClick={(e) => e.stopPropagation()}
            style={{ width: "420px", maxWidth: "90vw" }}
          >
            <div className="flex justify-between items-start mb-6">
              <h3 className="text-xl font-semibold text-foreground tracking-tight leading-tight pr-8">
            ...
            <div className="space-y-5 text-sm">
              {/* Course Info Grid */}
              <div className="grid grid-cols-2 gap-4 bg-muted/40 rounded-lg p-4 border border-border/50">
                <div>
                ...
                <div className="space-y-2">
                  {selectedCourse.sessions?.map((session, index) => {
                    ...
                    if (typeof session.day === "string") {
                      ...
                      dayName =
                        englishToSpanishDays[session.day] || session.day;
                    ...
                    return (
                      <div
                        key={index}
                        className="flex flex-col gap-1.5 p-3 rounded-lg border border-border/30 bg-background/50"
                      >
