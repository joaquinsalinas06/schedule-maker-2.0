...
export function EnhancedCollaborativeBuilder() {
  ...
  const loadCourseSelections = async () => {
    ...
    try {
      ...
      const selections = await CollaborationAPI.getCourseSelections(
        currentSession.id,
      );
  ...
  const addSection = async (course: Course, sectionId: number) => {
    ...
    const isAlreadySelected = courseSelections.some(
      (cs) =>
  ...
  const addCourseSelectionLocal = async (
    section: SelectedSection,
    selectionType: "shared" | "individual",
    sharedWith: number[] = [],
  ) => {
    ...
    if (currentSession?.id) {
      try {
        ...
        const savedSelection = await CollaborationAPI.saveCourseSelection(
          currentSession.id,
          apiData,
        );
  ...
  const removeCourseSelectionLocal = async (index: number) => {
    ...
    if (currentSession?.id && selectionToRemove?.id) {
      try {
        const { CollaborationAPI } =
          await import("@/services/collaborationAPI");
        await CollaborationAPI.removeCourseSelection(
          currentSession.id,
          selectionToRemove.id,
        );
      } catch (error) {
  ...
  const updateSelectionType = async (
    ...
    if (currentSession?.id && selection?.id) {
      try {
        const { CollaborationAPI } =
          await import("@/services/collaborationAPI");
        await CollaborationAPI.updateCourseSelection(
          currentSession.id,
          selection.id,
          {
            selection_type: newType,
        ...
        setCourseSelections(
          courseSelections.map((sel, i) => (i === index ? selection : sel)),
        );
        // Could show a toast notification here
      }
    }
  ...
  const getSharedCourses = () => {
    return courseSelections.filter(
      (selection) => selection.selection_type === "shared",
    );
  };

  const getIndividualCourses = () => {
    return courseSelections.filter(
      (selection) => selection.selection_type === "individual",
    );
  };
  ...
  const generateSchedules = async () => {
    ...
    try {
      ...
      const currentUserSchedule = {
        ...
        participantName:
          currentSession?.participants.find((p) => p.id === currentUserId)
            ?.name || "You",
      ...
      if (currentSession?.id) {
        try {
          ...
          const sectionIds = userSections
            .map((selection) => selection.schedule_data?.sectionId)
            .filter((id) => id);
  ...
  const transformScheduleData = (
    userSections: CollaborativeCourseSelection[],
  ) => {
    const courses = userSections.map((selection, index) => {
      return {
        ...
        sessions:
          selection.schedule_data?.sessions?.map(
    ...
    return (
      <div className="text-center py-12">
        ...
        <p className="text-muted-foreground">
          Join or create a collaborative session to start building schedules
          together.
        </p>
      </div>
    );
          ...
          <div className="flex justify-between items-start mb-4">
            <div>
              ...
              <p className="text-sm text-muted-foreground mt-1">
                {currentSession.participants.length} collaborators •{" "}
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-2">
              {currentSession.participants.map(
                (participant, participantIndex) => (
                  <div
                    key={`participant-${participant.id || participantIndex}-${participant.name || participantIndex}`}
                    className="flex items-center gap-2 text-sm p-2 rounded-md bg-muted/30"
                  >
                    <div
                      className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-medium ${getUserColor(participant.id)}`}
                    >
                    ...
                    <div
                      className={`w-2 h-2 rounded-full flex-shrink-0 ${
                        onlineUsers.some((u) => u.id === participant.id)
                          ? "bg-primary"
                          : "bg-muted"
                      }`}
                    />
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-6">
          ...
          <Card className="bg-card/80 backdrop-blur-sm border-border shadow-lg">
            <CardHeader>
              <div className="flex justify-end items-center">
                {/* Removed duplicate title - ScheduleVisualization has its own */}
                <Button
            ...
            <CardContent>
              {(() => {
                ...
                <div className="text-center py-24 text-muted-foreground">
                  ...
                  <p className="text-sm">
                    Add sections and generate your schedule to see it here
                  </p>
                  <p className="text-xs mt-2">
                    Current sections: {getSharedCourses().length} shared •{" "}
                    {getIndividualCourses().length} individual
                  </p>
                </div>
              )}
            </CardContent>
        ...
        <div className="xl:col-span-4 space-y-4">
          ...
          <Card className="bg-card/80 backdrop-blur-sm border-border shadow-lg">
            <CardHeader className="pb-3">
              ...
              <Button
                ...
                onClick={() => {
                  const newType =
                    activeSearchType === "shared" ? "individual" : "shared";
            ...
            <CardContent className="space-y-3">
              ...
              <div className="max-h-64 overflow-y-auto pr-2 scrollbar-thin scrollbar-thumb-primary scrollbar-track-transparent">
                {searchResults.length > 0 ? (
                  ...
                  <div className="text-center py-8 text-muted-foreground">
                    ...
                    <p className="text-xs mt-1">
                      Adding as:{" "}
                      <span className="font-medium">{activeSearchType}</span>
                    </p>
                  </div>
                )}
              </div>
          ...
          <Card className="bg-card/80 backdrop-blur-sm border-border shadow-lg">
            <CardHeader className="pb-3">
              <div className="flex items-center justify-between">
                ...
                <div className="text-xs text-muted-foreground">
                  {getSharedCourses().length + getIndividualCourses().length}{" "}
                  total
                </div>
              </div>
              ...
              <Button
                ...
                onClick={() => {
                  const newType =
                    activeTab === "shared" ? "individual" : "shared";
            ...
            <CardContent>
              <div className="max-h-96 overflow-y-auto pr-2 scrollbar-thin scrollbar-thumb-primary scrollbar-track-transparent">
                {activeTab === "shared" ? (
                  getSharedCourses().length === 0 ? (
                    <div className="text-center py-8 text-muted-foreground">
                      ...
                      <p className="text-xs">
                        Search and add sections to share with your team
                      </p>
                    </div>
                  ) : (
                    <div className="space-y-2">
                      {Object.entries(
                        groupCoursesByCode(getSharedCourses()),
                      ).map(([courseCode, courseSections]) => {
                        const isExpanded = expandedCourses.has(
                          `shared-${courseCode}`,
                        );
                        const usersForCourse =
                          getUsersForCourse(courseSections);
                        const courseName = courseSections[0]?.course_name || "";

                        return (
                          <div
                            key={`shared-${courseCode}`}
                            className="border rounded-lg bg-muted/30 border-border"
                          >
                            <div
                              className="flex items-center justify-between p-2 cursor-pointer hover:bg-muted/50 transition-colors"
                              onClick={() =>
                                toggleCourseExpansion(`shared-${courseCode}`)
                              }
                            >
                            ...
                            {isExpanded && (
                              <div className="border-t border-border bg-background/50">
                                {courseSections.map((course, index) => (
                                  <div
                                    key={`section-${index}`}
                                    className="p-2 border-b border-border last:border-b-0 flex items-center justify-between"
                                  >
                                    <div className="flex-1 min-w-0">
                                    ...
                                    <Button
                                      ...
                                      onClick={(e) => {
                                        e.stopPropagation();
                                        removeCourseSelectionLocal(
                                          courseSelections.indexOf(course),
                                        );
                                      }}
                  ...
                  <div className="text-center py-8 text-muted-foreground">
                    ...
                    <p className="text-xs">
                      Search and add sections for your personal schedule
                    </p>
                  </div>
                ) : (
                  <div className="space-y-2">
                    {Object.entries(
                      groupCoursesByCode(getIndividualCourses()),
                    ).map(([courseCode, courseSections]) => {
                      const isExpanded = expandedCourses.has(
                        `individual-${courseCode}`,
                      );
                      const courseName = courseSections[0]?.course_name || "";

                      return (
                        <div
                          key={`individual-${courseCode}`}
                          className="border rounded-lg bg-muted/30 border-border"
                        >
                          <div
                            className="flex items-center justify-between p-2 cursor-pointer hover:bg-muted/50 transition-colors"
                            onClick={() =>
                              toggleCourseExpansion(`individual-${courseCode}`)
                            }
                          >
                            <div className="flex items-center gap-2 flex-1 min-w-0">
                              {isExpanded ? (
      ...
      <SectionSelectionPopup
        ...
        selectedSections={courseSelections
          .filter(
            (cs) =>
              sectionPopup?.course &&
              cs.course_code === sectionPopup.course.code,
          )
          .map((cs) => ({
        ...
        removeSection={(index: number) => {
          ...
          const selectedSectionsForThisCourse = courseSelections.filter(
            (cs) =>
              sectionPopup?.course &&
              cs.course_code === sectionPopup.course.code,
          );
